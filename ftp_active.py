#!/usr/bin/env python3
"""
Quectel FOTA FTP Server - Active Mode for better embedded device compatibility
"""
import os
import logging
import shutil
...
class QuectelActiveFTPHandler(FTPHandler):
    # Increase timeout for slow embedded devices
    timeout = 300  # 5 minutes
    
    def __init__(self, conn, server, ioloop=None):
        super().__init__(conn, server, ioloop)
        # Force active mode for better embedded device compatibility
        self.passive_dtp = None
    ...
    def on_connect(self):
        ...
        print(f"[INFO] Using ACTIVE mode for data connections")
    ...
    def ftp_SIZE(self, line):
        ...
        try:
            ...
            # Show actual file size for debugging
            try:
                actual_path = self.fs.realpath(line)
                actual_size = os.path.getsize(actual_path)
                print(f"[SIZE] Actual file size on disk: {actual_size} bytes")
            except:
                pass
            return result
    ...
    def ftp_PORT(self, line):
        print(f"[PORT] Client setting active mode data connection: {line}")
        result = super().ftp_PORT(line)
        print(f"[PORT] Active mode data connection established")
        return result
    
    def ftp_PASV(self, line):
        print(f"[PASV] Client requesting passive mode (will force active instead)")
        # We could return an error to force active mode, but let's allow it
        result = super().ftp_PASV(line)
        print(f"[PASV] Passive mode connection established")
        return result
...
def main():
    ...
    for source_file in source_firmware_files:
        if os.path.exists(source_file):
            ...
            if not os.path.exists(dest_file):  # Don't overwrite existing files
                shutil.copy2(source_file, dest_file)
                print(f"Copied {source_file} to {dest_file}")
            copied_files.append((os.path.basename(source_file), os.path.getsize(dest_file)))
    ...
    # Create handler optimized for Quectel with active mode preference
    handler = QuectelActiveFTPHandler
    handler.authorizer = authorizer
    
    # Optimize for embedded devices - wider passive port range as fallback
    handler.passive_ports = range(60000, 60100)  # Wider range as fallback
    handler.banner = "Quectel FOTA Active Mode Server"
    ...
    print("=" * 60)
    print("QUECTEL FOTA FTP SERVER - ACTIVE MODE OPTIMIZED")
    print("=" * 60)
    ...
    print(f"\nData connection mode: ACTIVE (better for embedded devices)")
    print(f"Passive ports (fallback): 60000-60099")
